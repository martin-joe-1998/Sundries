...
void main()
{
	...
	float gbufferSpecular = texture(uGSpecular, gbufferCoord).r;
	...
	if (NdotL > 0)
	{
		...
		vec3 specularColor = mix(uPointLight.mDiffuseColor, vec3(0.0, 0.0, 0.0), intensity);
		specularColor *= pow(RdotV, gbufferSpecular);
		Phong += specularColor;
	}
