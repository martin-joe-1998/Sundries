...
{
	// **
	// inavailable controller, no response
	if (mControllerIndex < 0 || mControllerIndex >= MAX_CONTROLLERS) { return; }

	const ControllerState& controller = state.Controller[0];
	
	// Controller is not connected
	if (!controller.GetIsConnected()) { return; }
